...
export default function Careers() {
  ...
  const openings = [
    { title: t('careers.jobs.ai_engineer'), dept: t('careers.jobs.engineering'), location: 'Seoul / Remote', type: t('careers.fulltime') },
    { title: t('careers.jobs.frontend'), dept: t('careers.jobs.engineering'), location: 'Seoul / Remote', type: t('careers.fulltime') },
    { title: t('careers.jobs.backend'), dept: t('careers.jobs.engineering'), location: 'Seoul / Remote', type: t('careers.fulltime') },
    { title: t('careers.jobs.pm'), dept: t('careers.jobs.product'), location: 'Seoul', type: t('careers.fulltime') },
    { title: t('careers.jobs.sales'), dept: t('careers.jobs.salesDept'), location: 'Seoul / Remote', type: t('careers.fulltime') },
    { title: t('careers.jobs.sa'), dept: t('careers.jobs.cs'), location: 'Seoul / Remote', type: t('careers.fulltime') },
  ];
  ...
  const perks = [
    ...
    { icon: '📚', title: t('careers.perks.learning_title'), desc: t('careers.perks.learning_desc') },
  ];
    ...
    <div>
      ...
      <section className="py-20 px-4 sm:px-6" style={{ backgroundColor: '#0e0e11' }}>
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12" style={{ color: '#f1f1f3' }}>{t('careers.why')}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {perks.map((perk) => (
              <div key={perk.title} className="p-6 rounded-xl flex gap-4" style={{ backgroundColor: '#1a1a1f', border: '1px solid #2a2a33' }}>
                <span className="text-2xl">{perk.icon}</span>
                <div>
                  <h3 className="font-semibold mb-1" style={{ color: '#f1f1f3' }}>{perk.title}</h3>
                  <p className="text-sm" style={{ color: '#9ca3af' }}>{perk.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 px-4 sm:px-6" style={{ backgroundColor: '#111114' }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold mb-10" style={{ color: '#f1f1f3' }}>{t('careers.openings')}</h2>
          <div className="space-y-3">
            {openings.map((job) => (
              ...
              >
                <div>
                  <h3 className="font-semibold mb-1" style={{ color: '#f1f1f3' }}>{job.title}</h3>
                  <div className="flex flex-wrap gap-3">
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: `${deptColors[job.dept] ?? '#6b7280'}22`, color: deptColors[job.dept] ?? '#6b7280' }}>{job.dept}</span>
                    <span className="text-xs" style={{ color: '#6b7280' }}>{job.location}</span>
                    <span className="text-xs" style={{ color: '#6b7280' }}>{job.type}</span>
                  </div>
                </div>
                <button
      ...
      <section className="py-16 px-4 sm:px-6 text-center" style={{ backgroundColor: '#0e0e11' }}>
        <div className="max-w-xl mx-auto">
          <h2 className="text-2xl font-bold mb-4" style={{ color: '#f1f1f3' }}>{t('careers.noRole_title')}</h2>
          <p className="mb-8" style={{ color: '#9ca3af' }}>{t('careers.noRole_desc')}</p>
          <button
