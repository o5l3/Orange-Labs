...
export default function UserManual() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-24">
      ...
      <h1 className="text-4xl font-bold mb-4" style={{ color: '#f1f1f3' }}>{t('userManual.title')}</h1>
      <p className="text-lg mb-12" style={{ color: '#9ca3af' }}>
        {t('userManual.desc')}
      </p>
      <div className="grid sm:grid-cols-2 gap-6">
        {sections.map((section) => (
          <div key={section.title} className="p-6 rounded-2xl" style={{ backgroundColor: '#1a1a1f', border: '1px solid #2a2a33' }}>
            <h2 className="text-lg font-semibold mb-4" style={{ color: '#f1f1f3' }}>{section.title}</h2>
            <ul className="space-y-2">
              {section.items.map((item) => (
                <li key={item.key} className="flex items-center gap-2 text-sm cursor-pointer transition-colors"
                  style={{ color: '#9ca3af' }}
